    ...
    with driver.session() as session:
        ...
        for entity_type in ["Character", "Location", "Organization", "Artifact", "Event"]:
            ...
            result = session.run(f"""
                MATCH (e:{entity_type})
                RETURN e.name AS name, id(e) AS id, e.aliases AS aliases
            """)
            
            entities = [(record["name"], record["id"], record["aliases"] or []) for record in result]
                    ...
                    if use_apoc:
                        ...
                        try:
                            ...
                            session.run("""
                                ...
                                YIELD node
                                RETURN node
                            """, primary_id=primary_id, duplicate_id=node_id, duplicate_name=name)
